...
public void draw() {
  ...
  textSize(24);
  fill(255);
  text("Time Left: " + (10*generations-PApplet.parseInt(millis()/1000)), 10, 25);
  text("Best score Gen "+generations+": "+PApplet.parseInt(carSystem.topFitness) ,210, 75); 
...
public void simulate() {
  if (millis() > timer+simulationTimer*1000) {
    ...
    bestFitness = PApplet.parseInt(carSystem.topFitness);
    }
    carSystem.generate();
...
class CarSystem {
  ...
  float mutationRate = 0.05f; //Procent rate for hver enkel gen mutation så 11*5%
  float topFitness;
...
// 
  public void generate() { //Bruger copy metoden med mutation
